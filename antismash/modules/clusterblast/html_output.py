...
from antismash.common.json import JSONDataclass
...
@dataclass
class GeneMatchJSON(JSONDataclass):
...
@dataclass
class ReferenceDataJSON(JSONDataclass):
...
@dataclass
class ClusterblastVariantJSON(JSONDataclass):
...
@dataclass
class ClusterblastJSON(JSONDataclass):
...
def generate_javascript_data(record: Record, region: Region, results: ClusterBlastResults) -> dict[str, Any]:
