...
from antismash.common.secmet.locations import FeatureLocation, Location
...
def _build_location_from_prodigal(gene: ProdigalGene) -> Location:
    start = gene.start
    end = gene.end
    strand = gene.strand
...
def _filter_genes(genes_found: Iterable[ProdigalGene]) -> Iterable[Location]:
    for gene in genes_found:
        ...
        yield _build_location_from_prodigal(gene)
...
def run_prodigal(record: Record) -> None:
    ...
    """
    seq = str(record.seq)
    locations = _filter_genes(exec_prodigal(seq))
    count = 0
