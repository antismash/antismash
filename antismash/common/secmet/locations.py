...
def convert_protein_position_to_dna(start: int, end: int, location: FeatureLocation) -> Tuple[int, int]:
    ...
    for part in location.parts:
        ...
        if not start_found and dna_start < len(part) + processed:
            start_found = True
            if part.strand == -1:
                dna_start = part.end - dna_start + processed
            else:
                dna_start = part.start + dna_start - processed
        if not end_found and dna_end <= len(part) + processed:
            end_found = True
            if part.strand == -1:
                dna_end = part.end - dna_end + processed
            else:
                dna_end = part.start + dna_end - processed
        processed += len(part)

    assert start_found
    assert end_found

    if not location.start <= dna_start <= location.end:
        raise ValueError(("Converted coordinates %d..%d "
                          "out of bounds for location %s") % (dna_start, dna_end, location))

    if not location.start <= dna_end <= location.end:
        raise ValueError(("Converted coordinates %d..%d "
                          "out of bounds for location %s") % (dna_start, dna_end, location))
    ...
    # todo: this function will still fail on CompoundLocations, which have overlapping exons.
    # instead, consider merging this with get_sub_location_from_protein_coordinates()
