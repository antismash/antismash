...
def offset_location(location: Location, offset: int, *, wrap_point: int = None) -> Location:
    ...
    # if no offset is requested, just clone the location
    if not offset:
        if isinstance(location, CompoundLocation):
            return CompoundLocation(location.parts.copy(), operator=location.operator)
        return location

    # if no wrapping requested, just shift it
    if wrap_point is None:
    ...
    if len(location) == wrap_point:
        if isinstance(location, CompoundLocation):
            return CompoundLocation(location.parts, operator=location.operator)
        return FeatureLocation(location.start, location.end, strand=location.strand)
    ...
    return CompoundLocation(merged)
