...
def run_muscle_version() -> str:
    ...
    if not version_string.lower().startswith("muscle"):
    ...
    if "muscle 5" in version_string.lower():
        msg = "incompatible muscle binary version (version5): %s"
        raise RuntimeError(msg % muscle)
