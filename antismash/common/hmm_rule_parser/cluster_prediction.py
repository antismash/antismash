...
def merge_over_origin(clusters: list[Protocluster], record: Record) -> list[Protocluster]:
    ...
    for group in by_product.values():
        ...
        # as final step, check if the first and last ought to merge since sorting is awkward
        cluster = new_clusters[-1][0]
        if len(new_clusters) > 1 and cluster.overlaps_with(new_clusters[0][1]):
            prev_cluster, _ = new_clusters.pop(0)
            merged = merge_pair(prev_cluster, cluster)
            new_clusters[-1] = (merged, record.extend_location(merged.core_location, merged.cutoff))
