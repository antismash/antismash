            ...
            else
            {
                ...
                else
                {
                    ...
                    if (Alpaca.isArray(conditionalData)) {
                        ...
                        //    valid = false;
                        //}
                        if (dependentOnData != null)
                        {
                            //Format dependentOnData Array from incoming data that is either object or string
                            if (typeof dependentOnData == "object")
                            {
                                dependentOnDataArray = [];
                                for(var key in dependentOnData) {
                                    dependentOnDataArray.push(dependentOnData[key])
                                }
                            }
                            else if (typeof dependentOnData == "string")
                            {
                                var dependentOnDataArray = dependentOnData.split(",");
                            }

                            //Format conditionalData into array
                            conditionalDataArray = [];
                            for(var key in conditionalData) {
                                conditionalDataArray.push(conditionalData[key])
                            }

                            //Use arrays to check if at least one of the selected data items are in the conditional items
                            valid = false;
                            for (i = 0; i < dependentOnDataArray.length; i++) {
                                if ($.inArray(dependentOnDataArray[i], conditionalDataArray) != -1)
                                {
                                    valid = true;
                                };
                            };
