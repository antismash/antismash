...
class TestConversion(unittest.TestCase):
    def test_neighbourhood_bounded(self):
        ...
        result = jsonify.get_clusters_from_region(region, record_length=length, circular=False)
        types = [area["kind"] for area in result]
        # no subregions in the record, so they shouldn't be present
        # a single candidate with a single protocluster, so the candidate hidden/not present
        assert types == ["protocluster"]
        result = result[-1]
        ...
        result = jsonify.get_clusters_from_region(region, record_length=length, circular=True)
        types = [area["kind"] for area in result]
        assert types == ["protocluster"]
        result = result[-1]
