# Copyright (C) 2010-2018 Srikanth Duddela, Daniel Krug
...
def add_model(root, cluster):
    ...
    SubElement(genecluster, "type").text = cluster.get_product_string()
...
def populate_genelist(root, record, nodelist, domains, buildingblocks):
    ...
    for cds_feature in record.get_cds_features():
        ...
        # do not try to store domains or motifs for cds that do not belong to any cluster
        if not cds_feature.cluster:
            continue
...
def populate_motiflist(root, cds, record):
    ...
    for motif_feature in cds.motifs:
        ...
        SubElement(motif, "motif_name").text = motif_feature.motif
